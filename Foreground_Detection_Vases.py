...
def create_scaled_image(image):
    """Creates a scaled version of the image"""
    height = len(image)
    width = len(image[0])
...
def color_pair_m_prob(color1, color2, m_values):
    ...
    hue_prob = hyperbola_prediction(100*hue_diff, m_values[0]) * (1.0 - hyperbola_integral(m_values[0], 100*hue_diff)/hyperbola_integral(m_values[0], 20.0))
...
def find_foreground(image, m_values, known, min_prob):
    """Uses a breadth first search process to find the foreground of an image, given a model of
    the color changes between adjacent pixels; a set of known backgroud pixels; and a cutoff for
    how much adjacent background/foreground pixels differ."""
...
def smoothed_foreground(image_list, min_prob_2):
    """Finds the foreground of an image."""
    
    current_image = image_list[0]
    known = find_border(current_image)
    m_values = find_m_values(current_image)
    min_prob_1 = 0.01
    probs = find_foreground(current_image, m_values, known, min_prob_1)
    known_foreground = probs[1]
    height = len(known_foreground)
    width = len(known_foreground[0])
    smaller_dim = min(height, width)
    known_outcome = known_gaussian_smoothing(known_foreground, min_prob_2, float(smaller_dim)/50.0)
...
"""Loads an image and detects its foreground. Uncomment the code to run it. Change the number
to an integer in [1, 12] to see the foreground for a different vase."""

#image_to_parse = io.imread("Original_Vase_6.jpg")

#scaled_image_list = create_scaled_image(image_to_parse)

#foreground = smoothed_foreground(scaled_image_list, 0.9)
